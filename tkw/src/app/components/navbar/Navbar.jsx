...
const Navbar = () => {
  ...
  const toggleMenu = () => {
    setIsOpen((prev) => {
      ...
      controls.start({ x: newIsOpen ? "0%" : "-100%" });
      return newIsOpen;
    });
  };
  ...
  const handleClickOutside = (event) => {
    if (ref.current && !ref.current.contains(event.target) && isOpen) {
      setIsOpen(false);
      controls.start({ x: "-100%" }); // Close the menu
    }
  };

  useEffect(() => {
    window.addEventListener("click", handleClickOutside, true); // Use capture: true
    return () => {
  ...
  return (
    <nav
      className="fixed top-0 left-0 w-full mt-4 text-white bg-transparent z-20"
      ref={ref}
    >
      <div className="max-w-[1200px] mx-auto flex justify-between items-center px-8 py-4">
        ...
        <div className="hidden md:flex md:flex-row">
          ...
          <Link
            href="#cart"
            className="m-6 flex items-center hover:text-tertiary"
          >
            <svg
        ...
        <Link
          href="#cart"
          className="ml-24 md:hidden flex items-center hover:text-tertiary"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 20 20"
      ...
      <motion.div
        initial={{ x: "-100%" }}
        animate={controls}
        className="fixed top-0 left-0 w-3/4 h-full bg-black text-white shadow-lg z-40"
      >
        <div className="flex flex-col p-4 relative">
          <div className="flex items-center">
            ...
            <button
              className="pl-20 md:hidden text-white z-50"
              onClick={toggleMenu}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
